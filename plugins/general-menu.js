...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
   ...
   // conn.sendMessage(m.chat, { video: { url: 'https://telegra.ph/file/c82d5c358495e8ef15916.mp4' }, gifPlayback: true, gifAttribution: ~~(Math.random() * 2), caption: text.trim(), footer: await conn.getName(conn.user.jid) , templateButtons: [{ quickReplyButton: { displayText: 'Speedtest', id: `${_p}ping` }}, { quickReplyButton: { displayText: 'Owner', id: `${_p}owner` }} ] })
     /*conn.sendButton(m.chat, text.trim(), conn.user.name, await genProfile(conn, m), [['Speedtest', _p + 'ping'], ['stats', _p + 'stats']], m)*/
    conn.sendHydrated(m.chat, text.trim(), conn.user.name, 'https://telegra.ph/file/a44daf038cb12fce2f71d.png', 'https://chat.whatsapp.com/KDP1ybBbJafLEb5ZWVhXl6', 'GroupWA', null, null, [
      ['Speed', '/ping'],
      ['Stats', '/stats'],
      ['Listgc', '/listgc']
    //, m)
